...
@section('page-subtitle', 'Perbarui panduan atau solusi untuk pengguna')

@section('content')

<div class="max-w-4xl mx-auto">
    <form action="{{ route('manage.knowledge.update', $knowledge) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-2xl shadow-sm overflow-hidden mb-6">
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="title" name="title" value="{{ old('title', $knowledge->title) }}" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 focus:bg-white text-sm transition-colors outline-none">
                ...
                <div>
                    ...
                    <select id="category_id" name="category_id" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 focus:bg-white text-sm transition-colors outline-none">
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ old('category_id', $knowledge->category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <textarea id="content" name="content" required rows="12" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 focus:bg-white text-sm transition-colors outline-none resize-y">{{ old('content', $knowledge->content) }}</textarea>
                    @error('content') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                </div>

                <div class="flex items-center justify-between p-4 bg-slate-50/80 rounded-xl border border-slate-100">
                    <div>
                        <p class="text-sm font-semibold text-slate-800">Status Publikasi</p>
                        <p class="text-xs text-slate-500 mt-0.5">Jika diaktifkan, pengguna dapat melihat artikel ini di portal Knowledge Base.</p>
                    </div>
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="hidden" name="is_published" value="0">
                        <input type="checkbox" name="is_published" id="is_published" value="1" {{ old('is_published', $knowledge->is_published) ? 'checked' : '' }} class="sr-only peer">
